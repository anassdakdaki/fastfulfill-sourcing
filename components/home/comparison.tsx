...
function Cell({ value, highlight }: { value: string; highlight?: boolean }) {
  if (value === "yes") {
    return (
      <div className={`flex items-center justify-center gap-1.5 ${highlight ? "text-brand-600" : "text-gray-400"}`}>
        <CheckCircle2 size={16} className={highlight ? "text-brand-600" : "text-green-500"} />
  ...
  return (
    <span className={`text-sm font-medium ${highlight ? "text-brand-700 font-bold" : "text-gray-500"}`}>
      {value}
    </span>
  );
}
...
export function Comparison() {
  return (
    <section className="py-24 bg-gray-50" id="compare">
      <div className="container-section">
          ...
          <table className="w-full min-w-[680px]">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-500 w-40">Feature</th>
                <th className="px-6 py-4 text-center w-36">
                  <div className="inline-flex flex-col items-center gap-1">
                    <span className="text-xs font-black text-brand-600 uppercase tracking-widest bg-brand-50 border border-brand-200 px-3 py-1 rounded-full">
                      FastFulfill
                    </span>
                  </div>
                </th>
                <th className="px-6 py-4 text-center text-sm font-semibold text-gray-400 w-36">Zendrop</th>
                <th className="px-6 py-4 text-center text-sm font-semibold text-gray-400 w-36">CJ Drop</th>
                <th className="px-6 py-4 text-center text-sm font-semibold text-gray-400 w-36">WeChat Agent</th>
              </tr>
            </thead>
            <tbody>
              {ROWS.map((row, i) => (
        ...
        {/* Legend */}
        <div className="mt-4 flex items-center gap-6 justify-end text-xs text-gray-400">
          ...
          <span className="flex items-center gap-1.5"><XCircle size={13} className="text-gray-300" /> Not available</span>
        </div>
        ...
        <div className="mt-8 text-center">
          ...
          <p className="mt-3 text-xs text-gray-400">No credit card. No monthly fee. Just the quote.</p>
        </div>
      </div>
    </section>
  );
