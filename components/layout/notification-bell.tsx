          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-gray-50">
            {loading ? (
              ...
              notifications.map((n) => {
                ...
                const content = (
                  <div
                    className={`flex items-start gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50 transition-colors ${!n.read ? "bg-brand-50/40" : ""}`}
                    onClick={() => handleRead(n.id, n.link)}
                  >
                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${!n.read ? "bg-brand-100" : "bg-gray-100"}`}>
                      <Icon size={14} className={!n.read ? "text-brand-600" : "text-gray-500"} />
                    </div>
