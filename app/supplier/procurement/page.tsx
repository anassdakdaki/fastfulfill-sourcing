...
import { DUMMY_PROCUREMENT } from "@/lib/dummy-data";
...
const STATUS_NEXT: Record<ProcurementStatus, ProcurementStatus | null> = {
  sourcing:     "ordered",
  ordered:      "in_transit",
        ...
        return (
          <div key={step.key} className="flex items-center gap-1">
            <div className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium ${
              current ? `${step.bg} ${step.color} ring-1 ring-current/30` :
            ...
            {i < PIPELINE.length - 1 && (
              <ChevronRight size={11} className={i < idx ? "text-green-400" : "text-gray-200"} />
            )}
          </div>
        );
      })}
...
export default function ProcurementPage() {
  const [items, setItems] = useState<ProcurementItem[]>(DUMMY_PROCUREMENT);
    ...
    <div className="py-8 px-6 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Track every accepted deal from sourcing through to warehouse arrival. Supplier details are internal only.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-4 gap-3">
        {PIPELINE.map((step) => {
          ...
          return (
            <button
              key={step.key}
              onClick={() => setFilter((f) => f === step.key ? "all" : step.key)}
              className={`card p-4 text-left transition-all hover:shadow-md ${
                filter === step.key ? "ring-2 ring-brand-400" : ""
              }`}
            >
                ...
                <div className="flex items-start gap-4 flex-wrap">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-xs text-gray-500">
                      ...
                      {item.eta && (
                        <span>ETA: <strong className="text-gray-800">
                          {new Date(item.eta).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
                        </strong></span>
                      )}
                      {item.ordered_at && (
                        <span>Ordered: {new Date(item.ordered_at).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</span>
                      )}
                    </div>

                    {/* Supplier info (internal) */}
                    {showSupplier && (
                      <div className="mt-2 inline-flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5 text-xs">
                        ...
                        <span className="text-amber-600">· ${item.supplier_cost}/unit</span>
                      </div>
                    )}

                    {item.notes && (
                      <p className="mt-2 text-xs text-gray-400 italic">{item.notes}</p>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-3 shrink-0">
                    ...
                    {cta && (
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white text-xs font-semibold rounded-xl transition-colors"
                      >
                        {item.status === "sourcing" && <Loader2 size={13} />}
                        {item.status === "ordered"    && <Truck size={13} />}
                        {item.status === "in_transit" && <Warehouse size={13} />}
                        {cta}
                      </button>
                    )}
                  </div>
      ...
      {showSupplier && (
        <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 text-xs text-amber-700 flex items-start gap-2">
          ...
          <span>
            <strong>Supplier details are internal only.</strong> Seller names, product sources, and unit costs are
            never exposed to buyers. All communication goes through FastFulfill as the middleman.
          </span>
        </div>
      )}
    </div>
